...
export const action: ActionFunction = async ({ request }) => {
  ...
  const result = await request //
    ...
    .then((formData) => ({
      ...
      pieces: formData.getAll('pieces')?.map(title => ({
        id: uuidv4(),
        title,
        isDone: false,
      })),
  ...
  return (
    <CenterCardLayout>
      <Form method="post" className="flex flex-col">
        ...
        <input type="text" name="icon" value="commandLine" readOnly />
        <ErrorMessages errors={errors} name="icon" />
        <DynamicTextBoxList name="pieces"/>
        <ErrorMessages errors={errors} name="pieces" />
        <button type="submit" className="btn btn-primary mt-2" disabled={isCreating}>
